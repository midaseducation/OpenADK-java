// THIS FILE WAS AUTO-GENERATED BY ADKGEN -- DO NOT MODIFY!

//
// Copyright (c)1998-2011 Pearson Education, Inc. or its affiliate(s).
// All rights reserved.
//

package openadk.library.infra;

import openadk.library.*;

/**
 *  Defines the set of values that can be specified whenever a SIFProtocols
 *  is used as a parameter to a method or constructor. Alternatively, the static
 *  <code>wrap</code> method can be called to encapsulate any string value in
 *  a SIFProtocols object.<p>
 *
 *  @author Generated by adkgen
 *  @version 2.7
 *  @since 1.1
 */
public class SIFProtocols extends SIFEnum
{
	private static final long serialVersionUID = Element.CURRENT_SERIALIZE_VERSION;
	/**
	 *  HTTPS ("HTTPS")
	 */
	public static final SIFProtocols HTTP = new SIFProtocols("HTTPS");

	/**
	 *  HTTPS ("HTTPS")
	 */
	public static final SIFProtocols HTTPS = new SIFProtocols("HTTPS");

	/**
	 *  Wrap an arbitrary string value in a SIFProtocols object.
	 *  @param value The element/attribute value. This method does not verify
	 *      that the value is valid according to the SIF Specification.
	 */
	public static SIFProtocols wrap( String value ) {
		return new SIFProtocols( value );
	}

	private SIFProtocols( String value ) {
		super(value);
	}
}
